...
- 다국어는 번체 중국어(`zh-Hant`)를 사용한다. 현재 앱·작품·장소·지역 페이지의 언어 전환/폴백 규칙이 서로 달라, 통합 개발 Issue #10으로 관리한다.
...
3. #10 다국어 언어 전환·번역 준비상태 통합 설계 구현
4. #2 GitHub Projects 보드 생성 및 필드 설정
5. #3 코드 기준으로 기존 문서 6종의 실행 상태 확정 및 후속 Issue 분리
6. #5 아이디어·녹취·메일 Inbox 흐름 실제 파일럿
7. #8 대표 승인 기반 자동화: 선행 조건 충족 후 사용자의 “자동화하자” 명령으로 착수
